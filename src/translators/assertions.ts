...
export class CommandTranslatorAssertions extends CommandTranslatorTraversal {
  ...
  private async should(assertion: string, valueOrExpected?: any, expectedValue?: any): Promise<any> {
    ...
    const retryAssertion = async (handler: (...args: any[]) => Promise<void>): Promise<void> => {
      const timeout = 4000;
      const interval = 100;
    ...
    // Assertion handlers
    const handlers: Record<string, (subject: any, expected: any, expectedValue?: any, negate?: boolean) => Promise<void>> = {
      eq: async (subj, exp) => {
      ...
      a: async (subj, type) => {
        ...
        if (type === 'null') {
          ...
          const ctor = (globalThis as any)[type] || (window as any)?.[type];
          if (ctor) {
      ...
      an: async (subj, type) => {
        // Alias for 'a'
        const handler = (handlers as any).a;
        if (handler) {
      ...
      checked: async (subj) => {
        ...
        const tagName = await subj.__playwrightLocator.evaluate((el: any) => el.tagName?.toLowerCase());
        const inputType = await subj.__playwrightLocator.evaluate((el: any) => el.type?.toLowerCase());
      ...
      class: async (subj, className) => {
        let hasClass = false;
        if (subj && subj.__playwrightLocator) {
          hasClass = await subj.__playwrightLocator.evaluate((el: any, cls: string) => el.classList.contains(cls), className);
      ...
      property: async (subj, propName, propValue) => {
        ...
        const propVal = await subj.__playwrightLocator.evaluate((el: any, prop: string) => el[prop], propName);
      ...
      css: async (subj, cssProp, cssValue) => {
        ...
        const cssVal = await subj.__playwrightLocator.evaluate((el: any, prop: string) => window.getComputedStyle(el).getPropertyValue(prop), cssProp);
        const hasCss = !!cssVal;
      ...
      focused: async (subj) => {
        ...
        const isFocused = await subj.__playwrightLocator.evaluate((el: any) => document.activeElement === el);
      ...
      html: async (subj, expectedHtml) => {
        ...
        const html = await subj.__playwrightLocator.evaluate((el: any) => el.innerHTML);
      ...
      keys: async (subj, keys) => {
        ...
        const hasAll = keyList.every((k: string) => objKeys.includes(k));
      ...
      prop: async (subj, propName, propValue) => {
        ...
        const handler = (handlers as any).property;
        if (handler) {
      ...
      data: async (subj, dataKey, dataValue) => {
        ...
        const val = await subj.__playwrightLocator.evaluate((el: any, key: string) => (el as any).dataset?.[key] || (el as any).getAttribute(`data-${key}`), dataKey);
      ...
      selected: async (subj) => {
        ...
        const isSelected = await subj.__playwrightLocator.evaluate((el: any) => (el as any).selected === true);
      ...
      members: async (subj, expectedMembers) => {
        ...
        const hasAll = expected.every((item: any) => arr.includes(item));
        if (negate ? hasAll : !hasAll) {
  ...
  private async and(assertion: string, value?: any): Promise<any> {
