...
import type { VisitOptions, ScreenshotOptions, WaitOptions } from './types';
...
export class CommandTranslatorNavigation extends CommandTranslatorBase {
  ...
  private async visit(urlOrOptions: string | Partial<VisitOptions>, options?: Partial<VisitOptions>): Promise<void> {
    let url: string;
    ...
    const gotoOptions: any = { waitUntil: 'load', timeout: visitOptions.timeout || 60000 };
    ...
    if (visitOptions.onBeforeLoad) {
      await this.context.page.evaluate((fn: any) => {
        if (typeof fn === 'function') fn(window);
      }, visitOptions.onBeforeLoad as any);
    }
  ...
  private async viewport(width: number | string, height: number | string): Promise<void> {
  ...
  private async reload(forceReload?: boolean | object, options?: object): Promise<void> {
  ...
  private async go(direction: number | 'back' | 'forward', options?: object): Promise<void> {
    let steps = 1;
  ...
  private async location(key?: string): Promise<any> {
    if (key) {
      return this.context.page.evaluate((k: string) => (window as any).location[k], key);
    }
    return this.context.page.evaluate(() => window.location.href);
  }

  private async screenshot(filenameOrOptions?: string | Partial<ScreenshotOptions>, options?: Partial<ScreenshotOptions>): Promise<Buffer | null> {
  ...
  private async wait(msOrAlias: number | string | string[], options?: Partial<WaitOptions>): Promise<void> {
  ...
  private async log(messageOrOptions: string | { name?: string; message?: string; consoleProps?(): any }, value?: any): Promise<void> {
  ...
  private async window(options?: { log?: boolean; timeout?: number }): Promise<any> {
  ...
  private async document(options?: { log?: boolean; timeout?: number }): Promise<any> {
    const maxRetries = 3;
    for (let i = 0; i < maxRetries; i++) {
      try {
        return await this.context.page.evaluate(() => ({
