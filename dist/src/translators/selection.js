...
exports.CommandTranslatorSelection = void 0;
...
class CommandTranslatorSelection extends interaction_1.CommandTranslatorInteraction {
    ...
    async get(selectorOrAlias, options) {
    ...
    async contains(selectorOrText, textOrOptions, options) {
    ...
    async findByRole(role, options) {
        ...
        if (this.withinSubjectChain.length > 0 && options?.name) {
            locator = this.withinSubjectChain[this.withinSubjectChain.length - 1].getByRole(role, { name: options.name }).filter({ visible: true }).first();
        }
        else if (options?.name) {
            locator = this.context.page.getByRole(role, { name: options.name }).filter({ visible: true }).first();
        }
        else {
    ...
    async findAllByRole(role, options) {
        ...
        let locator;
        if (this.withinSubjectChain.length > 0 && options?.name) {
            locator = this.withinSubjectChain[this.withinSubjectChain.length - 1].getByRole(role, { name: options.name }).filter({ visible: true });
        }
        else if (options?.name) {
            locator = this.context.page.getByRole(role, { name: options.name }).filter({ visible: true });
        }
        else {
    ...
    async findByPlaceholderText(placeholder) {
        const locator = this.context.page.getByPlaceholder(placeholder).first();
        return this.createJQueryWrapper(locator);
    }
    async findByLabelText(label) {
...
exports.CommandTranslatorSelection = CommandTranslatorSelection;
